...
def main_driver_function():
    ...
    soup, driver = driver_connect("https://scholar.google.com/scholar?start=10&q=quantum+mechanics+applications+in+cancer+research&hl=en&as_sdt=0,5", 20)
    for a in soup.find_all(attrs = {'class' : 'gs_r gs_or gs_scl'}):
            
        title = a.find('h3', attrs = {'class' : 'gs_rt'})
        auther = a.find('div', attrs = {'class' : 'gs_a'})
    ...
    print(df)
    df.to_csv('results/extract_02.csv', index = True, encoding='utf-8')
